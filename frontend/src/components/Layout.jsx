...
const staffNavItems = [
  ...
  { to: "/ai-assistant", label: "AI", icon: <FiCpu className="h-5 w-5" aria-hidden /> },
...
const aiNavItem = { to: "/ai-assistant", label: "AI Assistant", icon: <FiCpu className="h-5 w-5" aria-hidden /> };
...
const NavLinkItem = ({ to, label, icon, collapsed = false }) => {
  const location = useLocation();
  const active = isActiveRoute(to, location.pathname, location.search);
  return (
    <NavLink
      to={to}
      className={`flex h-9 items-center rounded-lg text-sm font-medium transition-all duration-150 ${
        collapsed ? "gap-3 px-3 lg:justify-center lg:gap-0 lg:px-2" : "gap-3 px-3"
      } ${
        active
          ? "border-l-[3px] border-[#166534] bg-[#E9F6EE] text-[#166534] lg:border-l-0"
...
const ModuleGroup = ({ group, isExpanded, onToggle, collapsed = false, onOpen }) => {
  const location = useLocation();
  ...
  return (
    <div>
      <button
        ...
        className={`flex h-10 w-full items-center rounded-lg text-sm font-semibold transition-all duration-150 ${
          collapsed ? "gap-3 px-3 lg:justify-center lg:gap-0 lg:px-2" : "gap-3 px-3"
        } ${
          anyActive
            ? "border-l-[3px] border-[#166534] bg-[#E9F6EE] text-[#166534] lg:border-l-0"
            : "px-3 text-[#6B7280] hover:bg-[#F3F8F4] hover:text-[#166534]"
        }`}
      ...
      >
        ...
        {hasItems && (
          <span className={`shrink-0 transition-transform duration-200 ${collapsed ? "lg:hidden" : ""} ${isExpanded ? "rotate-180" : ""}`}>
      ...
      {hasItems && (
        <div
          className={`grid overflow-hidden transition-all duration-300 ease-in-out ${collapsed ? "lg:hidden" : ""} ${
  ...
  if (isManagement) {
    return (
      <div className="min-h-screen bg-[#f5f7fb] text-[#111827]">
        ...
        <aside
          className={`fixed inset-y-0 left-0 z-40 w-[calc(100%-2rem)] max-w-[280px] border-r border-[#E5E7EB] bg-white shadow-xl ${
            mobileMenuOpen ? "flex flex-col" : "hidden"
          } ${sidebarCollapsed ? "lg:w-20" : "lg:w-[280px]"} lg:z-30 lg:flex lg:max-w-none lg:flex-col lg:shadow-sm`}
        >
          <div className={`flex h-16 items-center justify-between gap-3 px-5 ${sidebarCollapsed ? "lg:gap-1 lg:px-2" : ""}`}>
            <img src="/logo.svg" alt="Logo" className={`h-9 w-9 ${sidebarCollapsed ? "lg:h-7 lg:w-7" : ""}`} />
            <div className={`mr-auto ${sidebarCollapsed ? "lg:hidden" : ""}`}>
            ...
            <button
              ...
              className={`hidden h-9 w-9 place-items-center rounded-lg text-[#6B7280] hover:bg-[#F3F8F4] hover:text-[#166534] lg:grid ${sidebarCollapsed ? "lg:h-7 lg:w-7" : ""}`}
              onClick={toggleSidebar}
            >
              {sidebarCollapsed ? <FiChevronRight className="h-5 w-5" aria-hidden /> : <FiChevronLeft className="h-5 w-5" aria-hidden />}
            </button>
            <button
              type="button"
              aria-label="Close navigation"
              className="grid h-9 w-9 place-items-center rounded-lg text-[#6B7280] hover:bg-[#F3F8F4] hover:text-[#166534] lg:hidden"
              onClick={() => setMobileMenuOpen(false)}
            >
          ...
          <nav className="sidebar-nav flex-1 space-y-1 overflow-y-auto px-3 py-4">
            {flatNavItems.map((item) => (
              <div key={item.to}>
                <NavLinkItem to={item.to} label={item.label} icon={item.icon} collapsed={sidebarCollapsed && !mobileMenuOpen} />
              </div>
            ))}
            ...
            <div>
              <NavLinkItem to={aiNavItem.to} label={aiNavItem.label} icon={aiNavItem.icon} collapsed={sidebarCollapsed && !mobileMenuOpen} />
            </div>
          ...
          <div className={`border-t border-[#E5E7EB] p-4 ${sidebarCollapsed ? "lg:p-3" : ""}`}>
            <div className={`flex items-center gap-3 rounded-xl bg-[#F9FAFB] p-3 ${sidebarCollapsed ? "lg:justify-center lg:p-2" : ""}`}>
            ...
            <button
              ...
              className={`mt-2 flex h-9 w-full items-center justify-center gap-2 rounded-lg text-sm font-semibold text-[#6B7280] transition-all hover:bg-[#F3F8F4] hover:text-[#166534] ${sidebarCollapsed ? "lg:gap-0" : ""}`}
            >
              <FiLogOut className="h-4 w-4" /> <span className={sidebarCollapsed ? "lg:hidden" : ""}>Logout</span>
            </button>
          </div>
        </aside>

        <div className={`flex min-h-screen ${sidebarCollapsed ? "lg:pl-20" : "lg:pl-[280px]"}`}>
              ...
              <div className="flex h-16 items-center justify-between gap-4 px-4 md:px-6">
                <div className="flex min-w-0 flex-1 items-center gap-3">
                  <button
                    ...
                    className="grid h-10 w-10 place-items-center rounded-lg border border-[#E5E7EB] text-[#6B7280] lg:hidden"
                ...
                <div className="flex items-center gap-3" data-ai-notification-anchor="account">
